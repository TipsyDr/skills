...
- 识别主线支线关联问题和伏笔回收情况
- 检查整体冲突点分布和节奏把控
- 记录**全局问题清单**到 `.webnovel/review/global-issues.json`

**第二阶段：章节细节审查（Agent Team 并行）**
- 使用多 Agent 并行处理，每个 Agent 负责一定数量的章节
- **⚠️ 遵循全局约束：每个子Agent最多负责3个章节**（详见 ? skill "子Agent并行处理规则"）
...
- 对每章进行详细审查：
  ...
  - 场景描写与对话质量评估
  - 伏笔设置与回收状态
- 生成**章节概要**到 `.webnovel/review/summaries/` 目录
- 记录**章节问题清单**到 `.webnovel/review/chapter-issues/` 目录
- 支持断点续传，已审查章节可跳过

**第三阶段：统一修复执行**
- 合并全局问题和章节问题，按严重程度排序（致命 > 严重 > 中等 > 轻微）
- 制定并执行**修复计划**，分为两种修复类型：
  - **轻量修复**（review 直接执行）：文字修正、段落调整、语句优化、字数填充等 minor 修改，**直接修改 `chapters/` 文件**（修改前自动备份到 `.webnovel/write/original/`）
...
- 生成**修复报告**到 `.webnovel/review/fix-report.json`
...
- **⚠️ 遵循全局约束：每个子Agent最多负责3个章节的审查**（详见 ? skill）
...
- **⚠️ 轻量修复阶段遵循全局约束：每个Agent最多负责3个章节**（详见 ? skill）

#### 修复机制说明
- **轻量级修复**：直接修改 `chapters/` 中有问题的段落或句子，修改前自动备份原始文件到 `.webnovel/write/original/`
- **重写修复**：对于严重有问题的章节，记录到 `fix-plan.json`，建议使用 `webnovel-write` 重新生成
- **自动备份**：修改 `chapters/` 文件前，自动将原始版本备份到 `.webnovel/write/original/`，确保可回滚
...
**第二阶段：章节细节审查输出**
- **章节概要目录**：每章核心剧情概要（`.webnovel/review/summaries/`）
...
**中间数据（仅系统内部使用，存于`.webnovel/review/`目录）**：
**第一阶段：全局审查数据**
...
#### 与其他 Skill 配合
- **前置 Skill**：读取 `webnovel-outline` 的大纲数据和 `webnovel-write` 的章节数据
   - 使用 `.webnovel/outline/` 的世界观、人设、大纲作为基准
   - **使用 `.webnovel/outline/chapter-outlines.json` 作为预期剧情参考**
   - 使用 `chapters/` 目录下的章节内容进行审查
   - 将实际章节内容与细纲进行对比，识别剧情偏离

- **修复计划输出**：修复完成后生成 `fix-plan.json`，其中标记了需要重写的章节及具体修复建议
   - worldbuilder 在编排流程时读取此文件，决定是否需要调用 `webnovel-write` 进行重写
   - 独立使用时，用户可查看 `fix-plan.json` 后手动调用 `webnovel-write` 重写指定章节

- **后续 Skill**：
   - **webnovel-write**：读取 `fix-plan.json`，执行重写修复（由 worldbuilder 编排或用户手动调用）
   - **webnovel-polish**：接收修复后的章节内容，进行文笔润色
   - **webnovel-finalize**：接收修复后的章节内容，进行完稿校验和多平台导出
...
合并问题清单（issues.json）
    ↓
第三阶段：统一修复
    ├─ 轻量修复 → 直接修改 chapters/（自动备份到 .webnovel/write/original/）
