...
### 核心功能
1. **基于完整细纲生成**：自动读取 `.webnovel/outline/chapter-outlines.json`，根据细纲批量生成章节
...
#### 自动读取细纲模式
当 `.webnovel/outline/chapter-outlines.json` 存在时，自动启用细纲驱动模式：
...
# 生成指定范围章节
/webnovel-write 第1-50章

# 生成特定卷的所有章节
/webnovel-write 第1卷

# 生成特定章节
/webnovel-write 第3章,第5章,第10章

# 批量并行创作指定范围
/webnovel-write 第1-100章 批量并行
```
...
```bash
# 使用自然语言描述生成单章
/webnovel-write 第3章 "主角在拍卖会上获得神秘功法"

# 基于已有细纲生成
/webnovel-write 第3章 按细纲生成
```
...
- ✅ 增量写入：每写完一章立即保存到`.webnovel/write/draft/`，无需等待全部完成
...
#### 中间过程数据（仅系统内部使用）
所有中间状态、元数据、进度信息统一保存到 `.webnovel/write/` 目录：
...
- `original/`：原始章节文件备份目录。当 review 或 polish 修改 `chapters/` 文件时，原始章节文件会先自动备份到 `.webnovel/write/original/` 目录，确保可回滚

#### 与其他 Skill 配合
- **前置 Skill**：自动读取 `.webnovel/outline/` 目录的大纲数据
  - 使用 `characters.json` 保持人物性格一致性
  - 使用 `chapter-outlines.json` 中的**完整章节细纲**驱动批量生成
  - 使用 `world.json` 保持世界观设定一致性
- **后续 Skill**：生成的章节数据可供 `webnovel-review`、`webnovel-polish`、`webnovel-finalize` 使用

#### 断点恢复
- 每次任务启动时读取 `chapters/` 目录下已存在的章节文件和 `.webnovel/write/progress.json` 进度
- 读取 `.webnovel/outline/chapter-outlines.json` 获取完整细纲
