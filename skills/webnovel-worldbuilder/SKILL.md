...
1. **选题策划**：调用 `webnovel-topic` 进行市场分析、选题定位，生成核心创意与卖点
2. **大纲设计**：调用 `webnovel-outline` 构建完整世界观、人物设定、分卷大纲与**完整章节细纲**
3. **内容创作**：调用 `webnovel-write` 按大纲进行分章节内容撰写，保持风格统一（**必须完成所有章节后才进入下一阶段**）
4. **逻辑审查**：调用 `webnovel-review` 对**所有已完成章节**进行全面审查，校验时间线一致性、人物行为逻辑、前后剧情连贯性
5. **内容润色**：调用 `webnovel-polish` 对**所有已审查章节**进行文笔优化、细节丰满、节奏调整
6. **完稿校验**：调用 `webnovel-finalize` 对**所有已润色章节**完成错别字检查、标点规范、逻辑漏洞最终排查与作品打包

### ⚠️ 全局约束：子Agent并行处理规则

worldbuilder 在协调所有涉及章节级操作的子技能时，强制执行 ? skill 中定义的**子Agent并行处理规则**：

**每个子Agent最多负责3个章节**（硬性约束，详见 ? skill "子Agent并行处理规则"）

此规则适用于以下所有阶段：
- **写作阶段**（webnovel-write）
- **审查阶段**（webnovel-review）：审查和轻量修复
- **润色阶段**（webnovel-polish）
- **完稿阶段**（webnovel-finalize）
- **细纲生成**（webnovel-outline）

**设计原因**：详见 ? skill "子Agent并行处理规则"
...
用户需求
    ↓
[收集创作需求 → 保存到 .webnovel/session/requirements.json]
    ↓
[webnovel-topic] 选题策划
    → 输出: 选题策划报告.md, .webnovel/topic/options.json
    ↓
[webnovel-outline] 大纲设计（**生成完整章节细纲**）
    → 输入: .webnovel/topic/options.json (可选)
    → 输出: 小说大纲_*.md, .webnovel/outline/*.json（含 chapter-outlines.json）
    ↓
[阶段检查点 1] 检查大纲完整性，确认章节细纲已生成
    ↓
[webnovel-write] 章节撰写（**细纲驱动，并行批量生成，每个Agent最多3章**）
    → 输入: .webnovel/outline/chapter-outlines.json
    → 输出: chapters/*.md, .webnovel/write/*.json
    → ✅ **必须完成所有章节**（检查章节数与细纲一致）
    → ⚠️ **遵循全局3章/Agent约束**
    ↓
[阶段检查点 2] 验证所有章节已完成，记录写入进度
    ↓
[webnovel-review] 逻辑审查（**审查所有章节，每个Agent最多3章**）
    → 输入: .webnovel/outline/*.json, chapters/*.md
    → 输出: 剧情审查报告.md, .webnovel/review/*.json, .webnovel/review/fix-plan.json
    → ✅ **必须完成全本审查**
    → ⚠️ **遵循全局3章/Agent约束**
    ↓
...
{fix-plan.json 有重写修复项?}
    →|是| [调用webnovel-write重写指定章节 → 直接修改 chapters/（自动备份到 .webnovel/write/original/）]
    →|否| 继续
    ↓
[review 轻量修复已直接修改 chapters/，无需额外应用步骤]
    ↓
[webnovel-polish] 内容润色（**润色所有章节，每个Agent最多3章**）
    → 输入: chapters/*.md, .webnovel/review/*.json
    → 输出: chapters/*.md（润色后直接修改，自动备份到 .webnovel/write/original/）, .webnovel/polish/*.json
    → ✅ **必须完成全本润色**
    → 📝 **润色结果直接修改 chapters/，修改前自动备份到 .webnovel/write/original/**
    → ⚠️ **遵循全局3章/Agent约束**
    ↓
[阶段检查点 4] 验证润色完成
    ↓
[webnovel-finalize] 完稿校验（**处理所有章节，每个Agent最多3章**）
    → 输入: chapters/*.md（已包含review修复和polish润色后的内容）
    → 输出: publish/*, .webnovel/finalize/*
    → ⚠️ **遵循全局3章/Agent约束**
```
...
- **审查阶段**：确认 `剧情审查报告.md` 生成且包含全本审查结果，确认 `fix-plan.json` 已生成。review 的轻量修复已直接修改 `chapters/`，修改前已自动备份到 `.webnovel/write/original/`
- **润色阶段**：确认润色已直接修改 `chapters/` 中的文件。润色修改前已自动备份到 `.webnovel/write/original/`
- **完稿阶段**：确认 `publish/` 目录下各平台格式导出完成

### 数据共享机制

所有 skill 通过 `.webnovel/` 目录共享数据：
...
flowchart LR
    ...
    H --> I[直接修改 chapters/，自动备份到 .webnovel/write/original/]
...
6. **审查阶段**：调用 `webnovel-review` 执行三阶段审查修复流程（遵循全局3章/Agent约束）：
    - **第一阶段：全局审查**：分析整体剧情脉络、时间线、设定一致性、冲突点分布、伏笔回收状态
    - **第二阶段：章节细节审查**：逐章检查字数、时间线、人物OOC、物品状态、场景质量、伏笔设置
    - **第三阶段：统一修复**：
      - 合并全局和章节问题，按严重程度制定修复计划
      - 执行**轻量修复**（文字修正、段落调整、字数填充等），直接修改 `chapters/` 文件（自动备份到 `.webnovel/write/original/`）
    ...
    - worldbuilder 读取 `fix-plan.json`，如有重写项则调用 `webnovel-write` 重写指定章节，直接修改 `chapters/`
    - 修复完成后重新验证，确保所有问题已解决
    - 完成后更新进度文件
7. **润色阶段**：调用 `webnovel-polish` 进行全本内容润色（遵循全局3章/Agent约束），提供多种润色风格选项（精简/详写/抒情/热血等）
    - 润色完成后，直接修改 `chapters/` 中的文件（修改前自动备份到 `.webnovel/write/original/`）
    - 润色过程和结果记录到 `.webnovel/polish/` 目录
    - 完成后更新进度文件
8. **完稿阶段**：调用 `webnovel-finalize` 对所有润色后的章节进行完稿校验（遵循全局3章/Agent约束），从 `chapters/` 目录读取最终内容（已包含review修复和polish润色后的结果），输出多种格式（Markdown/HTML/EPUB）到 `publish/` 目录，生成创作总结报告，完成后更新进度文件为最终完成状态
...
#### 需求确认机制
- 所有用户回答自动保存到 `.webnovel/session/user-requirements.json`，全流程各阶段共享使用
...
#### 数据持久化规范
所有中间状态数据统一存储在当前工作目录的 `.webnovel/` 目录下，避免上下文压缩或清理导致数据丢失：
...
```json
{
  ...
  "stages": {
    ...
    "outline": {
      ...
      "output_files": [
        "小说大纲_玄幻系统流.md",
        ".webnovel/outline/chapter-outlines.json"
      ]
    },
    "write": {
      "status": "completed",
    ...
    "review": {
      ...
      "output_files": [
        "剧情审查报告.md",
        ".webnovel/review/issues.json",
        ".webnovel/review/fix-plan.json"
      ],
      "statistics": {
...
4. **断点恢复**：恢复创作时自动从 `.webnovel/` 目录读取对应阶段数据
5. **清理规则**：支持清理所有中间数据，默认保留最近3个版本
6. **数据复用**：可直接引用其他项目的 `.webnovel/` 目录数据，实现世界观/人设复用
